--TEST--
Column drop: dropColumn operation and error cases
--SKIPIF--
...
?>
--EXPECT--
Dropped 'temp_val' column OK (removed from schema)
Other columns (id, score, v) still accessible after drop
Drop affects schema consistently
Correctly rejected dropping non-existent column
Dropped 'score' column OK
Schema correctly reflects dropped columns
Successfully re-added column with same name as dropped
Re-added column appears in schema
PASS: All dropColumn() scenarios work
